...
let blogName = "Kimkim Posts"; // The name of your blog, which will be shown in the footer and the title of the home page. You can also change the title of the home page by editing index.html if you want.
let authorName = "Kimkim";
...
/*Each time you make a new post, add the filepath here at the top of postsArray.
  ...
  below are expecting it ( 'blog/posts/YYYY-MM-DD-Title-of-Your-Post.html', ). You can
...
let postsArray = [
  [ "posts/2026-08-01-girlfriend.html" ],
  [ "posts/2026-07-11-old-IG-posts.html" ],
  [ "posts/2026-04-24-printf-hello,-world.html" ]
];
...
//Check if you are in blog (if so, the links will have to go up a directory)
let relativePath = ".";
...
let headerHTML = '<ul> <li><a href="' + relativePath + '/posts.html">Posts</a></li>' + 
'<li><a href="' + relativePath + '/archive.html">Archive</a></li>' +
'<li style="float: right;"><a href="/computer.html" style="text-decoration: none;">Back</a></li> </ul>';
...
let currentIndex = -1;
let currentFilename = url.substring(url.lastIndexOf('blog/posts/'));
...
function getPostFileInfo(filepath) {
  ...
  let match = filename.match(/^(\d{4})-(\d{2})-(\d{2})-(.+)\.html$/);
  if (match) {
    return {
      year: match[1],
      month: match[2],
...
function formatPostTitle(i) {
  ...
  if ( postsArray[i].length > 1 ) {
    ...
    let info = getPostFileInfo(postsArray[i][0]);
    if (info) {
      return info.slug.replace(/-/g, " ");
    }
...
function getMonthName(monthNum) {
  ...
  return monthNames[parseInt(monthNum, 10) - 1] || monthNum;
}
...
function formatPostLink(i) {
  ...
  if ( postsArray[i].length > 1 ) {
    ...
    let info = getPostFileInfo(postsArray[i][0]);
    if (info) {
      postTitle_i = info.slug.replace(/-/g," ");
    } else if ( postDateFormat.test ( postsArray[i][0].slice( 6,17 ) ) ) {
  ...
  let info = getPostFileInfo(postsArray[i][0]);
  if ( info ) {
    let dateString = info.year + " " + getMonthName(info.month) + " " + parseInt(info.day, 10);
    return '<li><a href="' + relativePath + '/'+ postsArray[i][0] +'">' + dateString + " \u00BB " + postTitle_i + '</a></li>';
  } else {
...
if ( postsArray.length < 2 ) {
  nextprevHTML = '<a href="' + relativePath + '/posts.html">Home</a>';
} else if ( currentIndex === 0 ) {
  prevlink = postsArray[currentIndex + 1][0];
  nextprevHTML = '<a href="' + relativePath + '/posts.html">Home</a> | <a href="'+ relativePath + '/' + prevlink +'">Previous Post \u00BB</a>';
} else if ( currentIndex === postsArray.length - 1 ) {
  nextlink = postsArray[currentIndex - 1][0];
  nextprevHTML = '<a href="' + relativePath + '/' + nextlink +'">\u00AB Next Post</a> | <a href="' + relativePath + '/posts.html">Home</a>';
} else if ( 0 < currentIndex && currentIndex < postsArray.length - 1 ) {
  nextlink = postsArray[currentIndex - 1][0];
  prevlink = postsArray[currentIndex + 1][0];
  nextprevHTML = '<a href="' + relativePath + '/'+ nextlink +'">\u00AB Next Post</a> | <a href="' + relativePath + '/posts.html">Home</a> | <a href="' + relativePath + '/'+ prevlink +'">Previous Post \u00BB</a>';
}
